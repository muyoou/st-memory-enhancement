...
import { debugWarn } from "./debugs.js";
...
try {
	...
	debugWarn(
		"未检测到 /scripts/custom-request.js 或未正确导出 ChatCompletionService，将禁用代理相关功能。",
		e
	);
}
	...
	constructor(config = {}) {
		...
		this.multiApiService = new MultiApiService({
			...
			system_prompt:
				...
				"",
			max_tokens:
				USER.tableBaseSetting?.custom_max_tokens ||
				config.max_tokens ||
				63000,
			...
			frequency_penalty:
				...
				0.0,
			presence_penalty:
				USER.tableBaseSetting?.custom_presence_penalty ||
