...
export class MultiApiService {
    ...
    buildRequestBody(messages, format) {
        switch (format) {
            case 'openai':
                ...
                // 添加可选参数
                if (this.config.top_p !== 1.0) openaiBody.top_p = this.config.top_p;
                if (this.config.frequency_penalty !== 0.0) openaiBody.frequency_penalty = this.config.frequency_penalty;
                if (this.config.presence_penalty !== 0.0) openaiBody.presence_penalty = this.config.presence_penalty;
            ...
            case 'gemini':
                ...
                // 添加Gemini特有参数
                if (this.config.top_p !== 1.0) geminiBody.generationConfig.topP = this.config.top_p;
                if (this.config.top_k !== 40) geminiBody.generationConfig.topK = this.config.top_k;
    ...
    parseResponse(responseData, format) {
        ...
        switch (format) {
            case 'openai':
                ...
                console.log('[MultiApiService] parseResponse: OpenAI内容', openaiContent);
            ...
            case 'gemini':
                ...
                // 检查是否有实际的文本内容
                const textParts = responseData.candidates[0].content.parts.filter(part => part.text && part.text.trim() !== '');
                ...
                if (textParts.length === 0) {
                    console.warn('[MultiApiService] parseResponse: Gemini没有有效文本内容');
                    return ""; // 返回空字符串而不是抛出错误
                }
                if (textParts.length > 1) {
                    const combinedText = textParts.map(part => part.text).join('');
                    console.log('[MultiApiService] parseResponse: Gemini合并多个文本部分', combinedText);
                    return combinedText;
                }
                const singleText = textParts[0].text;
                console.log('[MultiApiService] parseResponse: Gemini单个文本部分', singleText);
                return singleText;
            ...
            case 'claude':
                ...
                console.log('[MultiApiService] parseResponse: Claude内容', claudeContent);
                return claudeContent;
                
            default:
                // 默认使用OpenAI格式解析
                console.log('[MultiApiService] parseResponse: 使用默认OpenAI格式解析');
    ...
    async callLLM(prompt, streamCallback = null) {
        ...
        this.config.stream = streamCallback !== null && streamCallback !== undefined;
    ...
    async callDirectly(messages, streamCallback) {
        ...
        try {
            if (this.config.stream) {
                ...
                return await this.handleStreamResponse(apiEndpoint, headers, requestBody, format, streamCallback);
            } else {
                return await this.handleRegularResponse(apiEndpoint, headers, requestBody, format);
            }
        } catch (error) {
    ...
    async callWithProxy(messages, streamCallback) {
        ...
        try {
            ...
            throw new Error("当前酒馆版本过低，无法发送自定义请求。请更新你的酒馆版本");
        }
        ...
        try {
            const requestData = {
                ...
                model: this.config.model_name,
                temperature: this.config.temperature,
                chat_completion_source: this.config.api_format === 'openai' ? 'openai' : 'custom',
    ...
    async handleRegularResponse(apiEndpoint, headers, data, format) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`${format.toUpperCase()} API请求失败: ${response.status} - ${errorText}`);
        }
     ...
     */
    async handleStreamResponse(apiEndpoint, headers, data, format, streamCallback) {
        const response = await fetch(apiEndpoint, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify(data)
        ...
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`${format.toUpperCase()} API流式请求失败: ${response.status} - ${errorText}`);
        }
        ...
        try {
            ...
            while (true) {
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        console.warn(`[Stream] ${format.toUpperCase()}解析行错误:`, e, "行内容:", trimmedLine);
                    }
                }
            }
            ...
            console.error(`[Stream] ${format.toUpperCase()}流式读取错误:`, streamError);
            throw streamError;
        } finally {
            reader.releaseLock();
        }
    ...
    parseGeminiStreamLine(line) {
        if (line.startsWith('data: ')) {
            ...
            try {
                ...
                console.warn('[Stream] Gemini JSON parsing error:', e, 'Line:', dataStr);
            }
        }
        return '';
    }
    ...
    cleanResponse(text) {
        if (!text || typeof text !== 'string') {
            console.warn('[MultiApiService] cleanResponse: 输入为空或非字符串', text);
            return '';
        }
    ...
    async testConnection() {
        ...
        try {
            ...
            console.error(`测试${this.config.api_format.toUpperCase()} API连接错误:`, error);
            throw error;
        }
    }
}
