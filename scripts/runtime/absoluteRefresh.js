    ...
    try {
        ...
        if (USER.tableBaseSetting.separateReadLorebook && window.TavernHelper) {
            try {
                ...
                if (allEntries.length > 0) {
                    // --- Phase 1: Preparation & Constant Activation ---
                    const baseTriggerText = `${contextChats}\n${summaryChats}\n${finalPrompt}`;
                    const lowerBaseTrigger = baseTriggerText.toLowerCase();
                    const activatedUids = new Set();
                    let finalEntries = [];

                    const constantEntries = allEntries.filter(entry => entry.enabled && entry.type === 'constant');
                    ...
                    const constantEntriesCount = finalEntries.length;
                    const constantContent = constantEntries.filter(e => e.prevent_recursion !== true).map(e => e.content).join('\n');

                    // --- Phase 2: Initial Keyword Scan ---
                    let recursiveSeedText = '';
                    const initialTriggerText = `${baseTriggerText}\n${constantContent}`;
                    const lowerInitialTrigger = initialTriggerText.toLowerCase();
                    
                    const keywordEntries = allEntries.filter(entry => entry.enabled && entry.type !== 'constant' && Array.isArray(entry.keys) && entry.keys.length > 0);
                    const nonRecursiveKeywordEntries = keywordEntries.filter(entry => entry.exclude_recursion === true);
                    const recursiveKeywordEntries = keywordEntries.filter(entry => entry.exclude_recursion !== true);

                    // 2a. Scan non-recursive entries against ONLY the base context
                    for (const entry of nonRecursiveKeywordEntries) {
                        if (!activatedUids.has(entry.uid) && entry.keys.some(key => lowerBaseTrigger.includes(key.toLowerCase()))) {
                            finalEntries.push(entry);
                            activatedUids.add(entry.uid);
                        }
                    }
                    
                    // 2b. Scan recursive entries against base + constant content and build the recursive seed
                    for (const entry of recursiveKeywordEntries) {
                        if (!activatedUids.has(entry.uid) && entry.keys.some(key => lowerInitialTrigger.includes(key.toLowerCase()))) {
                            finalEntries.push(entry);
                            activatedUids.add(entry.uid);
                            if (entry.prevent_recursion !== true) {
                                recursiveSeedText += `\n${entry.content}`;
                            }
                        }
                    }
                    ...
                    while (newTriggersFound) {
                        ...
                        const fullTriggerText = `${initialTriggerText}\n${recursiveSeedText}`;
                        const lowerFullTrigger = fullTriggerText.toLowerCase();

                        const candidates = recursiveKeywordEntries.filter(entry => !activatedUids.has(entry.uid));

                        for (const entry of candidates) {
                            if (entry.keys.some(key => lowerFullTrigger.includes(key.toLowerCase()))) {
                                finalEntries.push(entry);
                                activatedUids.add(entry.uid);
                    ...
                    finalEntries.sort((a, b) => (a.order || 0) - (b.order || 0));
                    lorebookContent = finalEntries.map(entry => entry.content).join('\n');
                    console.log(`[Memory Enhancement] 最终激活了 ${finalEntries.length} 个世界书条目 (包含 ${constantEntriesCount} 个常量条目)。`);
                }
            } catch (e) {
        ...
        try {
            ...
            if (character && character.description) {
                // 如果 lorebookContent 不为空，则添加一个换行符作为分隔
                if (lorebookContent) {
                    lorebookContent += '\n\n';
                }
                lorebookContent += character.description;
                console.log('[Memory Enhancement] 已将角色描述附加到$4占位符内容中。');
            }
        } catch (e) {
            console.error('[Memory Enhancement] Error fetching or appending character description:', e);
        }
