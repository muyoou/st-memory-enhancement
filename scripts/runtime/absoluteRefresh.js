...
<div class="wide100p padding5 dataBankAttachments">
    ...
    <div id="tableRefresh" class="refresh-scroll-content">
        <div>
            <div class="operation-list-container"> ${content
				.map((table) => {
					return `
...
export async function rebuildTableActions(
	...
	try {
		...
		// console.log('原始模板systemPrompt:', systemPrompt);
		// console.log('原始模板userPrompt:', userPrompt);
		...
		try {
			...
			// console.log("JSON5解析成功 - systemPrompt类型:", typeof parsedSystemPrompt);
			// console.log("JSON5解析成功 - systemPrompt是否为数组:", Array.isArray(parsedSystemPrompt));
			// console.log("解析后的 systemPrompt:", parsedSystemPrompt);
		} catch (error) {
			// console.log("JSON5解析失败，尝试清理格式后重新解析:", error);
			...
			try {
				...
				// console.log("清理后的systemPrompt:", cleanedSystemPrompt);
				parsedSystemPrompt = JSON5.parse(cleanedSystemPrompt);
				// console.log("清理后JSON5解析成功 - systemPrompt类型:", typeof parsedSystemPrompt);
				// console.log("清理后JSON5解析成功 - systemPrompt是否为数组:", Array.isArray(parsedSystemPrompt));
			} catch (secondError) {
				// console.log("第二次JSON5解析仍然失败，使用原始字符串:", secondError);
				parsedSystemPrompt = systemPrompt;
				// console.log("解析失败后 - systemPrompt类型:", typeof parsedSystemPrompt);
				// console.log("解析失败后 - systemPrompt值:", parsedSystemPrompt);
			}
		}
		...
		if (typeof parsedSystemPrompt === "string") {
			...
			parsedSystemPrompt = parsedSystemPrompt.map((mes) => ({
				...mes,
				content: replacePrompt(mes.content),
			}));
		}
		...
		if (!userPrompt || userPrompt.trim() === "") {
			// console.log("userPrompt为空，使用默认值");
			userPrompt = `请你根据<整理规则>和<聊天记录>处理<当前表格>，并严格按照<当前表格>的格式回复我<新的表格>，回复务必使用中文，只回复<新的表格>的内容，不要回复多余的解释和思考：
			...
			try {
				...
				if (!silentUpdate) {
					...
					const confirmContent =
						confirmTheOperationPerformed(cleanContentTable);
				...
				if (piece) {
					for (const index in cleanContentTable) {
						...
						if (table.tableUid) {
							...
							const uid =
								DERIVED.any.waitingTableIdMap[table.tableIndex];
						...
						if (!sheet) {
							console.error(
								`无法找到表格 ${table.tableName} 对应的sheet`
							);
							continue;
						}
						const valueSheet = [
...
export async function rebuildSheets() {
	...
	previewDiv1.innerHTML = `<span>执行完毕后确认？：</span>${
	...
	previewDiv2.innerHTML = `<span>API：</span>${
...
export async function triggerStepByStepNow() {
	console.log(
		"[Memory Enhancement] Manually triggering step-by-step update..."
	);
...
export async function executeIncrementalUpdateFromSummary(
	...
	try {
		...
		if (USER.tableBaseSetting.separateReadLorebook && window.TavernHelper) {
			try {
				const charLorebooks =
					await window.TavernHelper.getCharLorebooks({ type: "all" });
				const bookNames = [];
				...
				for (const bookName of bookNames) {
					if (bookName) {
						const entries =
							await window.TavernHelper.getLorebookEntries(
								bookName
							);
				...
				console.error(
					"[Memory Enhancement] Error fetching lorebook content:",
					e
				);
			}
		}
		...
		console.log(
			"[Memory Enhancement] Step-by-step summary: Parsing and using multi-message template string."
		);
		...
		try {
			...
			if (!Array.isArray(promptMessages) || promptMessages.length === 0) {
				throw new Error(
					"Parsed prompt is not a valid non-empty array."
				);
			}
		} catch (e) {
			console.error(
				"Error parsing step_by_step_user_prompt string:",
				e,
				"Raw string:",
				stepByStepPromptString
			);
			EDITOR.error(
		...
		console.log(
			"Step-by-step: Prompts constructed from parsed multi-message template and sent as an array."
		);

		// 打印将要发送到API的最终数据
		if (Array.isArray(systemPromptForApi)) {
			console.log(
				"API-bound data (as message array):",
				systemPromptForApi
			);
			const totalContent = systemPromptForApi
				.map((m) => m.content)
				.join("");
			console.log(
				"Estimated token count:",
				estimateTokenCount(totalContent)
			);
		} else {
			...
			console.log(
				"Estimated token count:",
				estimateTokenCount(
					systemPromptForApi + (userPromptForApi || "")
				)
			);
		}
