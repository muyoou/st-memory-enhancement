...
function getSwipeUid(chat) {
	...
	if (!(swipeUid in chat.two_step_waiting))
...
function MarkChatAsWaiting(chat, swipeUid) {
	debugLog(USER.getContext().chat);
	debugLog("chat.two_step_links:", chat.two_step_links);
	debugLog("chat.two_step_waiting:", chat.two_step_waiting);
	chat.two_step_waiting[swipeUid] = true;
}
...
export async function TableTwoStepSummary(mode) {
	...
	if (todoPiece === undefined) {
		debugLog("未找到待填表的对话片段");
		EDITOR.error("未找到待填表的对话片段，请检查当前对话是否正确。");
		return;
	}
	let todoChats = todoPiece.mes;

	debugLog("待填表的对话片段:", todoChats);

	if (USER.tableBaseDefaultSettings.confirm_before_execution) {
		debugLog(
			"这里是" + USER.tableBaseDefaultSettings.confirm_before_execution
		);
		...
		if (confirmResult === false) {
			...
			if (confirmResult === "dont_remind_active") {
				debugLog("独立填表弹窗已被禁止，自动执行。");
				EDITOR.info("已选择“一直选是”，操作将在后台自动执行..."); // <--- 增加后台执行提示
			} else {
				debugLog('用户确认执行独立填表 (或首次选择了"一直选是"并确认)');
			}
...
export async function manualSummaryChat(todoChats, confirmResult) {
	...
	if (
		initialPiece.hash_sheets &&
		Object.keys(initialPiece.hash_sheets).length > 0
	) {
		debugLog(
			"[Memory Enhancement] 立即填表：检测到表格中有数据，执行恢复操作..."
		);
		try {
			await undoSheets(0);
			EDITOR.success("表格已恢复到上一版本。");
			debugLog("[Memory Enhancement] 表格恢复成功，准备执行填表。");
		} catch (e) {
			EDITOR.error("恢复表格失败，操作中止。", e.message, e);
			debugError("[Memory Enhancement] 调用 undoSheets 失败:", e);
			return;
		}
	} else {
		debugLog(
			"[Memory Enhancement] 立即填表：检测到为空表，跳过恢复步骤，直接执行填表。"
		);
	}
	...
	// 设置
	const useMainApiForStepByStep =
		USER.tableBaseSetting.step_by_step_use_main_api ?? true;
