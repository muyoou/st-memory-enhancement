...
function initChatScopeSelectedSheets() {
	const newSelectedSheets = BASE.sheetsData.context
		.map((sheet) => (sheet.enable ? sheet.uid : null))
		.filter(Boolean);
...
function initializeSelect2Dropdown(dropdownElement) {
	$(dropdownElement).select2({
		...
		templateResult: function (data) {
			...
			var $wrapper = $(
				'<span class="select2-option" style="width: 100%"></span>'
			);
			var $checkbox = $(
	...
	// 创建父级复选框与下拉框的关联
	const firstOptionText = $(dropdownElement)
		.find("option:first-child")
		.text();
	const tableMultipleSelectionDropdown = $(
		'<span class="select2-option" style="width: 100%"></span>'
	);
	const checkboxForParent = $(
		'<input type="checkbox" class="select2-option-checkbox"/>'
	);
	...
	const select2MultipleSelection = $(dropdownElement).next(
...
export function updateSelectBySheetStatus() {
	const templates = getSheets();
	const selectedSheetsUid = templates
		.filter((temp) => temp.enable)
		.map((temp) => temp.uid);
...
function bindSheetSetting(sheet, index) {
	...
	settingButton.on("click", async () => {
		...
		await popup.show();
		if (popup.result) {
			const diffData = compareDataDiff(
				formInstance.result(),
				initialData
			);
			debugLog(diffData);
	...
	const nameSpan = $(
		`<span style="margin-left: 0px;">#${index} ${
	...
	const sendToContextCheckbox = $(`
        <label class="checkbox_label" style="margin-left: 10px; font-weight: normal; color: var(--text_primary);">
            <input type="checkbox" class="send_to_context_switch" ${
				sheet.sendToContext !== false ? "checked" : ""
			} />
            <span data-i18n="Send to context">发送到上下文</span>
        </label>
    `);

	sendToContextCheckbox
		.find(".send_to_context_switch")
		.on("change", function () {
...
function bindCellClickEvent(cell) {
	cell.on("click", async (event) => {
		...
		if (rowIndex === 0 && colIndex === 0) {
			cell.parent.currentPopupMenu.add(
				'<i class="fa fa-arrow-right"></i> 向右插入列',
				(e) => {
					handleAction(cell, Cell.CellAction.insertRightColumn);
				}
			);
			if (
				sheetType === SheetBase.SheetType.free ||
				sheetType === SheetBase.SheetType.static
			) {
				cell.parent.currentPopupMenu.add(
					'<i class="fa fa-arrow-down"></i> 向下插入行',
					(e) => {
			...
			cell.parent.currentPopupMenu.add(
				'<i class="fa fa-arrow-left"></i> 向左插入列',
				(e) => {
			...
			if (
				...
				cell.parent.currentPopupMenu.add(
					'<i class="fa fa-arrow-up"></i> 向上插入行',
					(e) => {
...
async function updateDragTables() {
	...
	table_editor_container.querySelector(
		"#contentContainer"
	).style.outlineColor = scope === "chat" ? "#cf6e64" : "#41b681";
	...
	selectedSheetUids.forEach((uid, index) => {
		...
		if (scope === "chat") {
			...
			sheetDataExists = BASE.sheetsData.context?.some(
				(sheetData) => sheetData.uid === uid
			);
		} else {
			// 检查 uid 是否存在于 BASE.templates
			sheetDataExists = BASE.templates?.some(
				(templateData) => templateData.uid === uid
			);
		}
		...
		const tableElement = sheet.renderSheet(
			bindCellClickEvent,
			sheet.hashSheet.slice(0, 1)
		);
...
async function initTableEdit(mesId) {
	table_editor_container = $(
		await SYSTEM.getTemplate("sheetTemplateEditor")
	).get(0);
	const tableEditTips =
		table_editor_container.querySelector("#tableEditTips");
	const tableContainer =
		table_editor_container.querySelector("#tableContainer");
	const contentContainer =
		table_editor_container.querySelector("#contentContainer");
	const scopeSelect = table_editor_container.querySelector(
		"#structure_setting_scope"
	);
