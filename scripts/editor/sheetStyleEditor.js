...
import {
	initializeText,
	parseSheetRender,
	loadValueSheetBySheetHashSheet,
} from "../renderer/sheetCustomRenderer.js";
import { debugLog, debugWarn, debugError } from "../../services/debugs.js";
...
function refreshEditor() {
	// debugLog("refreshEditor-elements.rendererDisplay 是否存在:", !!elements.rendererDisplay);
	// debugLog("jQuery 对象长度:", elements.rendererDisplay?.length || 0);
	renderHTML();
	updateGuideContent(
		elements,
		dom.getValue(elements.matchMethod) === "regex"
	);
	dom.toggleVisibility(
		elements.table_renderer_display_container,
		dom.isChecked(elements.tablePreviewButton)
	);
	dom.toggleVisibility(
		elements.styleEnabledView,
		dom.isChecked(elements.tableStyleButton)
	);
}

// function renderHTML() {
//     const currentConfig = collectConfigThenUpdateTemplate();
//     debugLog("测试", currentConfig, templateInstance)
...
function renderHTML() {
	...
	templateInstance.tableSheet =
		loadValueSheetBySheetHashSheet(templateInstance);
...
async function getUIElements($dlg) {
	return {
		...
		table_renderer_display_container: $dlg.find(
			"#table_renderer_display_container"
		),
		match_method_regex_container: $dlg.find(
			"#match_method_regex_container"
		),
		push_to_chat_style_edit_guide_content: $dlg.find(
			"#push_to_chat_style_edit_guide_content"
		),
...
function updateGuideContent(elements, isRegex) {
	...
	elements.push_to_chat_style_edit_guide_content.html(
		isRegex
...
function getCurrentSelectedStyle() {
	if (
		!templateInstance.config.customStyles ||
		Object.keys(templateInstance.config.customStyles).length === 0
	) {
	...
	return (
		templateInstance.config.customStyles[selectedKey] ||
		templateInstance.config.customStyles[
			Object.keys(templateInstance.config.customStyles)[0]
		] ||
		DEFAULT_STYLE
	);
}
...
function setupSheetPreview() {
	if (!templateInstance) {
		debugWarn("setupSheetPreview: 未能获取到有效的 table 对象。");
		return;
	}

	// 初始化样式预览表格
	templateInstance.element = null;
	templateInstance.element = `<div class="justifyLeft scrollable">${
		templateInstance.renderSheet((cell) => {
			cell.element.style.cursor = "default";
		}).outerHTML
	}</div>`;
	// debugLog("setupSheetPreview-elements.rendererDisplay 是否存在:", !!elements.rendererDisplay);
	// debugLog("jQuery 对象长度:", elements.rendererDisplay?.length || 0);
	renderHTML();
...
function renderPreview() {
	try {
		...
		debugError("Preview rendering error:", e);
	}
}
...
function initUIValues() {
	// 初始化复选框
	dom.setChecked(
		elements.tableToChatButton,
		templateInstance.config.toChat !== false
	);
	dom.setChecked(
		elements.tableStyleButton,
		templateInstance.config.useCustomStyle !== false
	);
	dom.setChecked(
		elements.triggerSendToChatButton,
		templateInstance.config.triggerSendToChat !== false
	);
	dom.setChecked(
		elements.alternateTableButton,
		templateInstance.config.alternateTable == true
	);
	dom.setChecked(
		elements.insertTableButton,
		templateInstance.config.insertTable == true
	);
	dom.setChecked(
		elements.skipTopButton,
		templateInstance.config.skipTop == true
	);
	dom.setChecked(elements.tablePreviewButton, false);
	dom.setValue(
		elements.alternateLevel,
		templateInstance.config.alternateLevel || 0
	);
...
function initPresetStyleDropdown() {
	...
	if (
		...
		// 添加所有自定义样式
		Object.keys(templateInstance.config.customStyles).forEach(
			(styleName) => {
		...
		// 设置选中项
		if (
			templateInstance.config.selectedCustomStyleKey &&
			templateInstance.config.customStyles[
				templateInstance.config.selectedCustomStyleKey
			]
		) {
			...
			const firstStyleKey = presetDropdown
				.find("option:first")
				.get(0).value;
...
function bindEvents() {
	// 绑定基本输入元素事件
	["input", "input", "change", "change", "change", "change"].forEach(
		(eventType, i) => {
			[
				elements.regex,
				elements.replace,
				elements.tablePreviewButton,
				elements.matchMethod,
				elements.benchmark,
				elements.tableStyleButton,
			][i]
				.get(0)
				.addEventListener(eventType, refreshEditor);
		}
	);
...
function bindStyleManagementEvents() {
	...
	elements.addStyleButton.get(0).addEventListener("click", async function () {
		const styleName = await EDITOR.callGenericPopup(
			"输入新样式名称：",
			EDITOR.POPUP_TYPE.INPUT
		);
		if (!styleName) return;
	...
	elements.editStyleButton
		...
		.addEventListener("click", async function () {
			...
			const newName = await EDITOR.callGenericPopup(
				"修改样式名称：",
				EDITOR.POPUP_TYPE.INPUT,
				selectedKey
			);
			...
			// 重命名样式
			templateInstance.config.customStyles[newName] =
				templateInstance.config.customStyles[selectedKey];
			delete templateInstance.config.customStyles[selectedKey];

			// 更新下拉菜单
			const option = elements.presetStyle
				.find(`option[value="${selectedKey}"]`)
				.get(0);
			option.text = newName;
	...
	elements.deleteStyleButton
		...
		.addEventListener("click", async function () {
			...
			delete templateInstance.config.customStyles[selectedKey];
			elements.presetStyle
				.find(`option[value="${selectedKey}"]`)
				.remove();
			dom.setValue(
				elements.presetStyle,
				elements.presetStyle.find("option:first").get(0).value
			);
			dom.triggerEvent(elements.presetStyle, "change");
		});

	// 导入样式
	elements.importStyleButton
		.get(0)
		.addEventListener("click", async function () {
			const importData = await EDITOR.callGenericPopup(
				"粘贴样式配置JSON：",
				EDITOR.POPUP_TYPE.INPUT,
				"",
				{ rows: 10 }
...
function bindPreviewAndCopyEvents() {
	...
	elements.previewStyleButton.get(0).addEventListener("click", async () => {
		const currentConfig = collectConfigThenUpdateTemplate();
		const selectedStyle =
			currentConfig.customStyles[currentConfig.selectedCustomStyleKey];
			...
			.map(
				(option) =>
					`<option value="${option.value}" ${
						option.value === benchmarkValue ? "selected" : ""
					}>${option.text}</option>`
			)
			.join("");
		...
		setTimeout(() => {
			$dlg.find("#preview_benchmark_selector").on("change", function () {
				...
				$dlg.find("#table_to_chat_text_preview").val(
					initializeText(templateInstance, selectedStyle)
				);
			});
		}, 0);
	});
	...
	elements.copyTextButton
		...
		.addEventListener("click", () =>
			navigator.clipboard
				.writeText(elements.rendererDisplay.html())
...
export async function openSheetStyleRendererPopup(originInstance) {
	...
	const tableRendererPopup = new EDITOR.Popup(
		manager,
		EDITOR.POPUP_TYPE.CONFIRM,
		"",
		{
	...
	if (tableRendererPopup.result) {
		...
		const numberBoollen =
			isNaN(alternateLevel) ||
			alternateLevel < 0 ||
			Number.isInteger(alternateLevel) === false; //是否满足非负整数
		const styleBoollen = styleBasedOn.includes(
			finalConfig.customStyles[finalConfig.selectedCustomStyleKey].basedOn
		); //方式必须为html、csv、json、array
		if (numberBoollen || (alternateLevel > 0 && !styleBoollen)) {
			//输入的插入层级必须为非负整数，且不能为MarkDown格式否则改为0
			finalConfig.alternateLevel = 0;
			EDITOR.warning(
				"穿插层级必须为非负整数，且不能为MarkDown格式，否则强制改为0"
			);
		}
		Object.assign(originInstance.config, finalConfig);
		debugLog("表格样式已更新", originInstance.config.alternateLevel);
		originInstance.save();
