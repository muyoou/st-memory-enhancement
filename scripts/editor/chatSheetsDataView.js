...
export async function copyTable() {
	copyTableData = JSON.stringify(
		getTableJson({ type: "chatSheets", version: 1 })
	);
	if (!copyTableData) return;
	$("#table_drawer_icon").click();

	EDITOR.confirm(
		`正在复制表格数据 (#${SYSTEM.generateRandomString(4)})`,
		"取消",
		"粘贴到当前对话"
	).then(async (r) => {
		if (r) {
...
async function pasteTable() {
	if (USER.getContext().chat.length === 0) {
		EDITOR.error(
			"没有记录载体，表格是保存在聊天记录中的，请聊至少一轮后再重试"
		);
		return;
	}
	...
	if (confirmation) {
		if (copyTableData) {
			...
			if (!tables.mate === "chatSheets")
				return EDITOR.error("导入失败：文件格式不正确");
		...
		if (files && files.length > 0) {
			...
			reader.onload = async function (loadEvent) {
				...
				const popup = new EDITOR.Popup(
					...
					EDITOR.POPUP_TYPE.CONFIRM,
					"",
					{ okButton: "导入模板及数据", cancelButton: "取消" }
				);
				...
				if (result) {
					...
					else BASE.applyJsonToChatSheets(tables);
					await renderSheetsDOM();
					EDITOR.success("导入成功");
				}
			};
...
async function exportTable() {
	...
	const bom = "\uFEFF";
	const blob = new Blob([bom + JSON.stringify(jsonTables)], {
...
function getTableJson(mate) {
	if (
		...
		DERIVED.any.renderingSheets.length === 0
	) {
...
async function clearTable(mesId, viewSheetsContainer) {
	if (mesId === -1) return;
	const confirmation = await EDITOR.callGenericPopup(
		"清空当前对话的所有表格数据，并重置历史记录，该操作无法回退，是否继续？",
		EDITOR.POPUP_TYPE.CONFIRM,
		"",
		{ okButton: "继续", cancelButton: "取消" }
	);
...
function setTableEditTips(tableEditTips) {
	/* if (!tableEditTips || tableEditTips.length === 0) {
	       debugError('tableEditTips is null or empty jQuery object');
	       return;
    }
...
async function cellDataEdit(cell) {
	const result = await EDITOR.callGenericPopup(
		"编辑单元格",
	...
	if (result) {
		...
		refreshContextView();
		if (cell.type === Cell.CellType.column_header)
...
`;
	const columnCellDataPopup = new EDITOR.Popup(
		columnEditor,
		EDITOR.POPUP_TYPE.CONFIRM,
		"",
		{ okButton: "应用修改", cancelButton: "取消" }
	);
...
function batchEditMode(cell) {
	...
	EDITOR.confirm(`正在编辑 #${cell.parent.name} 的行`, "取消", "完成").then(
		(r) => {
...
export function cellClickEditModeEvent(cell) {
	...
	if (cell.type === Cell.CellType.row_header) {
		...
		const containerDiv = $(
			`<div class="flex-container" style="display: flex; flex-direction: row; justify-content: space-between; width: 100%;"></div>`
		);
		const rightDiv = $(
			`<div class="flex-container" style="margin-right: 3px"></div>`
		);
		const indexDiv = $(
			`<span class="menu_button_icon interactable" style="margin: 0; padding: 0 6px; cursor: move; color: var(--SmartThemeBodyColor)">${cell.position[0]}</span>`
		);
		const lockDiv = $(
			`<div><i class="menu_button menu_button_icon interactable fa fa-lock" style="margin: 0; border: none; color: var(--SmartThemeEmColor)"></i></div>`
		);
		const deleteDiv = $(
			`<div><i class="menu_button menu_button_icon interactable fa fa-xmark redWarningBG" style="margin: 0; border: none; color: var(--SmartThemeEmColor)"></i></div>`
		);
			...
			cell.parent.hashSheet.forEach((row) => {
				if (row[0] === cell.uid) {
					row.forEach((hash) => {
						...
						target.element.style.backgroundColor = target.locked
							? "#00ff0022"
							: "";
					});
				}
...
async function confirmAction(event, text = "是否继续该操作？") {
	const confirmation = new EDITOR.Popup(text, EDITOR.POPUP_TYPE.CONFIRM, "", {
		okButton: "继续",
		cancelButton: "取消",
	});
...
function cellClickEvent(cell) {
	...
	cell.on("click", async (event) => {
		...
		if (rowIndex === 0 && colIndex === 0) {
			menu.add(
				'<i class="fa-solid fa-bars-staggered"></i> 批量行编辑',
				() => batchEditMode(cell)
			);
			menu.add('<i class="fa fa-arrow-right"></i> 向右插入列', () =>
				handleAction(cell, Cell.CellAction.insertRightColumn)
			);
			menu.add('<i class="fa fa-arrow-down"></i> 向下插入行', () =>
				handleAction(cell, Cell.CellAction.insertDownRow)
			);
			menu.add(
				'<i class="fa-solid fa-wand-magic-sparkles"></i> 自定义表格样式',
				async () => customSheetStyle(cell)
			);
		} else if (colIndex === 0) {
			menu.add(
				'<i class="fa-solid fa-bars-staggered"></i> 批量行编辑',
			...
			menu.add('<i class="fa fa-arrow-up"></i> 向上插入行', () =>
			...
			menu.add(
				'<i class="fa fa-trash-alt"></i> 删除行',
				() => handleAction(cell, Cell.CellAction.deleteSelfRow),
				menu.ItemType.warning
			);
		} else if (rowIndex === 0) {
			menu.add(
				'<i class="fa fa-i-cursor"></i> 编辑该列',
				async () => await cellDataEdit(cell)
			);
			menu.add('<i class="fa fa-arrow-left"></i> 向左插入列', () =>
			...
			menu.add(
				'<i class="fa fa-i-cursor"></i> 编辑该单元格',
				async () => await cellDataEdit(cell)
			);
			menu.add(
				'<i class="fa-solid fa-clock-rotate-left"></i> 单元格历史记录',
				async () => await cellHistoryView(cell)
			);
		}
...
export async function renderEditableSheetsDOM(
	_sheets,
	_viewSheetsContainer,
	_cellClickEvent = cellClickEvent
) {
	for (let [index, sheet] of _sheets.entries()) {
		...
		if (DERIVED.any.batchEditMode === true) {
			if (DERIVED.any.batchEditModeSheet?.name === instance.name) {
				sheetElement = await instance.renderSheet(
					cellClickEditModeEvent
				);
			} else {
...
async function undoTable(mesId, tableContainer) {
	...
	const popup = new EDITOR.Popup(
		"撤销指定轮次内的所有手动修改及重整理数据，恢复表格",
		EDITOR.POPUP_TYPE.CONFIRM,
		"",
		{ okButton: "撤销本轮", cancelButton: "取消" }
	);
	const result = await popup.show();
	if (result) {
		await undoSheets(0);
...
async function renderSheetsDOM(mesId = -1) {
	...
	task.log();
	const { deep: lastestDeep, piece: lastestPiece } =
		BASE.getLastSheetsPiece();
	const { piece, deep } =
		mesId === -1
			? { piece: lastestPiece, deep: lastestDeep }
			: { piece: USER.getContext().chat[mesId], deep: mesId };
	...
	debugLog("renderSheetsDOM:", piece, sheets);
	DERIVED.any.renderingSheets = sheets;

	task.log();
	$(viewSheetsContainer).empty();
	viewSheetsContainer.style.paddingBottom = "150px";
	renderEditableSheetsDOM(
		sheets,
		viewSheetsContainer,
		DERIVED.any.isRenderLastest ? undefined : () => {}
	);
	$("#table_indicator").text(
		DERIVED.any.isRenderLastest
			? "现在是可修改的活动表格"
			: `现在是第${deep}轮对话中的旧表格，不可被更改`
	);
	task.log();
}
...
async function initTableView(mesId) {
	...
	$(document).on("click", "#table_next_button", function () {
		const deep = DERIVED.any.renderDeep;
		debugLog("当前深度：", deep);
		const { deep: nextDeep } = BASE.getLastSheetsPiece(
			deep + 1,
			20,
			false,
			"down"
