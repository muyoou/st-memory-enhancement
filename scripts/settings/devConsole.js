...
function updateTableDebugLog(type, message, detail = "", timeout, stack) {
	...
	switch (type) {
		...
		case "success":
			...
			break;
		case "warning":
			debugWarn(message, detail);
			toastr.warning(message, detail, { timeOut: timeout });
			break;
		case "error":
			debugError(message, detail);
...
async function copyPopup(log) {
	...
	const textarea = $(
		'<textarea class="log-copy-textarea" style="height: 100%"></textarea>'
	).val(logDetails);
	const manager = await SYSTEM.getTemplate("popup");
	const copyPopupInstance = new EDITOR.Popup(
		manager,
...
function renderDebugLogs($container, logs, onlyError) {
	...
	if (!logs || logs.length === 0) {
		$container.append(
			'<div class="debug-log-item">No debug log found.</div>'
		);
		return;
	}
	...
	logs.forEach((log) => {
		...
		const timeSpan = $('<span class="log-time"></span>').text(
			`[${log.time}]`
		);
		const typeSpan = $('<span class="log-type"></span>')
			.addClass(`log-type-${log.type}`)
			.text(`[${log.type}]`);
		const messageSpan = $('<span class="log-message"></span>').text(
			log.message
		);
		...
		if (log.stack) {
			...
			const formattedStack = log.stack.replace(
				stackLineRegex,
				(match, functionName, urlBase, lineNumber, columnNumber) => {
			...
			const stackPre = $('<pre class="log-stack"></pre>').html(
				formattedStack
			);
...
export const consoleMessageToEditor = {
	info: (message, detail, timeout) =>
		updateTableDebugLog("info", message, detail, timeout),
	success: (message, detail, timeout) =>
		updateTableDebugLog("success", message, detail, timeout),
	warning: (message, detail, timeout) =>
		updateTableDebugLog("warning", message, detail, timeout),
...
export async function openTableDebugLogPopup() {
	...
	const tableDebugLogPopup = new EDITOR.Popup(
		manager,
		EDITOR.POPUP_TYPE.TEXT,
		"",
		{ large: true, wide: true, allowVerticalScrolling: true }
	...
	renderDebugLogs(
		$debugLogContainer,
		debugEventHistory,
		$onlyErrorLogCheckbox.is(":checked")
	);

	$onlyErrorLogCheckbox.off("change").on("change", function () {
		// 移除之前的事件监听，避免重复绑定
	...
	$exportInfoButton.on("click", () => {
		const infoData = {
			...
			allHash: USER.getContext().chat.map(
				(chat) => chat.hash_sheets ?? null
			),
		};
		...
		input.onchange = async (event) => {
			...
			reader.onload = async (e) => {
				try {
					...
					lastestPiece.hash_sheets =
						json.lastestSheet.piece.hash_sheets;
					debugLog("导入的调试信息：", json);
					USER.saveChat();
				} catch (error) {
					debugError("导入失败：", error);
				}
			};
