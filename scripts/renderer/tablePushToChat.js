...
function divideCumstomReplace(replace, _viewSheetsContainer) {
	...
	// debugLog('分离后的函数内的样式数据:', dividedContent);
 ...
 */
function insertCustomRender(
	tableRole,
	insertMark,
	cycleMark,
	indexForTableRole,
	_sheets,
	_viewSheetsContainer
) {
	let customStyle = "";
	let index = 0;
	for (let i = 0; i < tableRole.length; i++) {
		index = indexForTableRole[i];
		debugLog("穿插及嵌入渲染表格角色索引：" + index);
		debugLog(
			_sheets[index].name,
			"穿插及嵌入渲染表格角色：" + tableRole[i]
		);
		...
		debugLog(
			"穿插及嵌入渲染表格角色赋值给sheet：",
			_sheets[index].name,
			_sheets[index].tableSheet
		);

		const customContent = parseSheetRender(_sheets[index]);
		debugLog("穿插及嵌入渲染表格返回文本customContentt：" + customContent);
	...
	// debugLog("穿插及嵌入最终返回文本customStyle：" + customStyle);
...
function ordinarycustomStyleRender(sheet, _viewSheetsContainer) {
	// debugLog('普通表格数据:', sheet.tableSheet);
...
function defaultStyleRender(index, sheet, _viewSheetsContainer) {
	...
	sheetElement = instance.renderSheet(
		(cell) => (cell.element.style.cursor = "default")
	);
...
function cycleJudge(cycleDivideMark, indexForRowAlternate, i) {
	if (i < 0) return false;
	return (
		cycleDivideMark[indexForRowAlternate[i]] === true &&
		cycleDivideMark[indexForRowAlternate[i + 1]] === true &&
		indexForRowAlternate[i] === indexForRowAlternate[i + 1]
	);
}
...
async function renderEditableSheetsDOM(_sheets, _viewSheetsContainer) {
	...
	debugLog("穿插模式是否开启：" + USER.tableBaseSetting.alternate_switch);
	if (USER.tableBaseSetting.alternate_switch) {
		//首先判断是否开启了穿插模式，再看是否有必要进入穿插模型
		for (let [index, sheet] of _sheets.entries()) {
			if (sheet.config.useCustomStyle === true) {
				_sheets[index].config.customStyles[
					sheet.config.selectedCustomStyleKey
				].replaceDivide = divideCumstomReplace(
					sheet.config.customStyles[
						sheet.config.selectedCustomStyleKey
					].replace,
					_viewSheetsContainer
				); //对CSS代码进行整理使得最后的文本更符合html格式
			}
			if (
				sheet.config.toChat === true &&
				sheet.config.useCustomStyle === true &&
				sheet.config.alternateTable === true &&
				sheet.config.alternateLevel > 0
			) {
				sumAlternateLevel++; // 符合条件的计数器增加
				levelIndexAlternate.push([
					Number(sheet.config.alternateLevel),
					index,
				]); // 加入层级和索引对应数组，强制转换成数字类型，提高健壮性
				sheet.config.skipTop = false; //穿擦模式只对表格内容进行渲染，且不需要跳过header行
				cycleDivideMark[index] =
					sheet.config.customStyles[
						sheet.config.selectedCustomStyleKey
					].replace.includes("<cycleDivide>");
	...
	if (sumAlternateLevel > 0) {
		...
		// debugLog('初始的层级索引对应：', levelIndexAlternate);
		levelIndexAlternate.sort((a, b) => {
			// 保证稳定排序
			if (a[0] !== b[0]) {
		...
		for (const [level, index] of levelIndexAlternate) {
			const sheetData = loadValueSheetBySheetHashSheet(
				_sheets[index]
			).slice(1);
		...
		let j = 0; //标记用变量
		// debugLog("排序后的表格：", tableAlternate);
		// 穿插+合并表格的渲染
		for (let i = 0; i < tableAlternate.length; i++) {
			// debugLog('当前行：', i, tableAlternate[i][1])
			if (i === tableAlternate.length - 1) {
				if (
					cycleJudge(cycleDivideMark, indexForRowAlternate, i - 1) ||
					cycleJudge(cycleDivideMark, indexForRowAlternate, i)
				) {
					...
					insertMark[j] =
						_sheets[indexForRowAlternate[i]].config.insertTable;
					cycleMark[j] = false;
				}
				// debugLog('最后一行提取结束：', j, tableAlternate[i][1])
				// debugLog('最后一行提取结束tableRole', tableRole);
				insertCustomRender(
					tableRole,
					insertMark,
					cycleMark,
				...
				if (
					...
					if (!tableRole[j]) {
						...
						cycleMark[j] = true;
						// debugLog('循环标记开始', j, i);
						// debugLog('循环标记开始的tableRole：', tableRole);
					}
					tableRole[j].push(tableAlternate[i]);
					...
					insertMark[j] =
						_sheets[indexForRowAlternate[i]].config.insertTable;
					cycleMark[j] = false;
					// debugLog('非循环输入提取：', _sheets[indexForRowAlternate[i]].name, j, i);
					j++;
				}
			} else {
				if (
				...
				// debugLog('同名行提取结束：', j, tableAlternate[i][1])
				// debugLog('同名行提取结束tableRole', tableRole);
				insertCustomRender(
					tableRole,
					insertMark,
					cycleMark,
		...
		// 对普通表格进行渲染
		// debugLog('普通表格的索引:', indexOriginary, '普通表格的长度', indexOriginary.length);
		for (let i = 0; i < indexOriginary.length; i++) {
			...
			// debugLog('进行普通渲染当前普通表格内容：',sheet.tableSheet);
...
function replaceTableToStatusTag(sheets) {
	...
	if (USER.tableBaseSetting.table_to_chat_mode === "context_bottom") {
		...
		chatContainer = window.document
			.querySelector(".last_mes")
			?.querySelector(".mes_text"); // 获取最后一条消息的容器
	...
	setTimeout(async () => {
		...
		const currentTableStatusContainer = document.querySelector(
			"#tableStatusContainer"
		);
		if (currentTableStatusContainer) {
			// 移除之前的事件监听器，防止重复添加 (虽然在这个场景下不太可能重复添加)
			currentTableStatusContainer.removeEventListener(
				"touchstart",
				touchstartHandler
			);
			currentTableStatusContainer.removeEventListener(
		...
		const r = USER.tableBaseSetting.to_chat_container.replace(
			/\$0/g,
			`<tableStatus id="table_push_to_chat_sheets"></tableStatus>`
		);
		$(chatContainer).append(
			`<div class="wide100p" id="tableStatusContainer">${r}</div>`
		); // 添加新的 tableStatusContainer
		const tableStatusContainer = chatContainer?.querySelector(
			"#table_push_to_chat_sheets"
		);
		renderEditableSheetsDOM(sheets, tableStatusContainer);

		// 获取新创建的 tableStatusContainer
		const newTableStatusContainer = chatContainer?.querySelector(
			"#tableStatusContainer"
		);
		if (newTableStatusContainer) {
			// 添加事件监听器，使用具名函数
			newTableStatusContainer.addEventListener(
				"touchstart",
				touchstartHandler,
				{ passive: false }
			);
		...
		// debugLog('tableStatusContainer:', newTableStatusContainer);
	}, 0);
}
...
export function updateSystemMessageTableStatus(force = false) {
	debugLog(
		"更新最后一条 System 消息的 <tableStatus> 标签内容",
		USER.tableBaseSetting.isTableToChat
	);
	if (force === false) {
		if (
			USER.tableBaseSetting.isExtensionAble === false ||
			USER.tableBaseSetting.isTableToChat === false
		) {
			window.document.querySelector("#tableStatusContainer")?.remove();
			return;
		}
	}
	// debugLog("更新最后一条 System ")
	const sheets = BASE.hashSheetsToSheets(
		BASE.getLastSheetsPiece()?.piece.hash_sheets
	);
...
export async function openTableRendererPopup() {
	...
	const tableRendererPopup = new EDITOR.Popup(
		...
		"",
		{ large: true, wide: true, allowVerticalScrolling: true }
	);
	const sheetsData = BASE.getLastSheetsPiece()?.piece.hash_sheets;
	if (!sheetsData) {
		// debugWarn("openTableRendererPopup: 未能获取到有效的 table 对象。");
		return;
	}
	...
	// 修改中实时渲染
	debugLog(
		"openTableRendererPopup-elements.rendererDisplay 是否存在:",
		!!elements.rendererDisplay
	);
