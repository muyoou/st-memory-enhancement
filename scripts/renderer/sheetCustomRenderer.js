...
function staticPipeline(target) {
	...
	if (!regexReplace || regexReplace === "")
		return target?.element || "<div>表格数据未加载</div>";
	if (!target) return regexReplace;

	// 新增：处理 {{GET::...}} 宏
	regexReplace = regexReplace.replace(
		/{{GET::\s*([^:]+?)\s*:\s*([A-Z]+\d+)\s*}}/g,
		(match, tableName, cellAddress) => {
			...
			try {
				...
				return cellValue !== undefined
					? cellValue
					: `<span style="color: orange">[GET: 在 "${tableName}" 中未找到单元格 "${cellAddress}"]</span>`;
			} catch (error) {
				debugError(
					`Error resolving GET macro for ${tableName}:${cellAddress}`,
					error
				);
...
export function loadValueSheetBySheetHashSheet(instance) {
	if (!instance) return;
	return instance.hashSheet.map((row) =>
		row.map((hash) => {
...
function regexReplacePipeline(text) {
	...
	// debugLog("分离后的替换文本：", replaceString)
	...
	try {
		...
		let result = "";
		let cycleReplace = processedReplaceString.match(
			/<cycleDivide>([\s\S]*?)<\/cycleDivide>/
		); //获取循环替换字符串

		if (cycleReplace) {
			let cycleReplaceString = cycleReplace[1]; //不含cycleDivide标签
			const cycleReplaceRegex = cycleReplace[0]; //含cycleDivide标签
			// debugLog("进入循环替换，获取的循环替换字符串：", '类型：', typeof cycleReplaceString, '内容：', cycleReplaceString);
			processedReplaceString = processedReplaceString.replace(
				cycleReplaceRegex,
				"regexTemporaryString"
			); //临时替换循环替换字符串
			cycleReplaceString = text.replace(regex, cycleReplaceString); //按正则替换循环字符串代码
			// debugLog("循环替换后的字符串：", cycleReplaceString);
			result = processedReplaceString.replace(
				"regexTemporaryString",
				cycleReplaceString
			);
		} else {
			...
			if (
				selectedCustomStyle.basedOn !== "html" &&
				selectedCustomStyle.basedOn !== "csv"
			) {
				//增加条件不是CSV格式的文本，目前测试出CSV使用该代码会出现渲染错误
				result = result.replace(/\n/g, "<br>");
			}
		}
		return result;
	} catch (error) {
		debugError("Error in regex replacement:", error);
...
function triggerValueSheet(valueSheet = [], skipTop, alternateTable) {
	...
	let triggerArray = [];
	let i = 0;
	// debugLog("上个聊天内容lastchat：", lastchat);
	// debugLog("valueSheet为：", valueSheet);
	// debugLog("valueSheet第1行为：", valueSheet[0]);
	// debugLog("判定前triggerArray为：", triggerArray);
	if (!alternateTable && !skipTop) {
		i = 1;
	}
	// debugLog("触发数组triggerArray为：", triggerArray, "\ni为：",i);
	for (i; i < valueSheet.length; i++) {
		// debugLog("触发词是：", valueSheet[i][1], "类型为：", typeof valueSheet[i][1]);
...
export function initializeText(target, selectedStyle) {
	let initialize = "";
	// debugLog("瞅瞅target是："+target.config.triggerSendToChat); //调试用，正常不开启
	let valueSheet = target.tableSheet; // 获取表格数据，二维数组
	// debugLog(target.name,"初始化文本表格：" , valueSheet);
	// 新增，判断是否需要触发sendToChat
	if (target.config.triggerSendToChat) {
		// debugLog(target.name + "开启触发推送" + valueSheet);
		valueSheet = triggerValueSheet(
			valueSheet,
			target.config.skipTop,
			target.config.alternateTable
		);
		// debugLog(target.name + "检索后valueSheet是否为数组：" + Array.isArray(valueSheet) + "\n检索后valueSheet最后是什么：" + valueSheet);
	}
	...
	switch (method) {
		...
		case "json":
			...
			break;
		default:
			debugError("不支持的格式:", method);
	}
	// debugLog('初始化值:', method, initialize);
	return initialize;
}
...
function regexPipeline(target, selectedStyle = selectedCustomStyle) {
	...
	// debugLog(target.name,"初始化文本：", initText);
	let result = selectedStyle.replace || "";
	// debugLog("初始化文本的长度", result.length);
...
function executeRendering(target) {
	...
	if (config.useCustomStyle === false) {
		...
		throw new Error(
			"未启用自定义样式，你需要在 parseSheetRender 外部排除 config.useCustomStyle === false 的情况"
		);
	}
...
export function parseSheetRender(instance, rendererConfig = undefined) {
	...
	selectedCustomStyle =
		config.customStyles[config.selectedCustomStyleKey] || {};
