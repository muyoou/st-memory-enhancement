...
} from "./scripts/renderer/tablePushToChat.js";
...
} from "./scripts/renderer/appHeaderTableBaseDrawer.js";
...
export function buildSheetsByTemplates(targetPiece) {
	...
	templates.forEach((template) => {
		...
		if (
			!template ||
			!template.hashSheet ||
			!Array.isArray(template.hashSheet) ||
			template.hashSheet.length === 0 ||
			!Array.isArray(template.hashSheet[0]) ||
		...
		try {
			...
			EDITOR.error(
				`[Memory Enhancement] 从模板创建或保存 sheet 时出错:`,
				error.message,
				error
			);
		}
...
export function convertOldTablesToNewSheets(oldTableList, targetPiece) {
	...
	for (const oldTable of oldTableList) {
		const valueSheet = [oldTable.columns, ...oldTable.content].map(
			(row) => ["", ...row]
		);
		const cols = valueSheet[0].length;
		const rows = valueSheet.length;
		const targetSheetUid = BASE.sheetsData.context.find(
			(sheet) => sheet.name === oldTable.tableName
		)?.uid;
		if (targetSheetUid) {
			// 如果表格已存在，则更新表格数据
 ...
 * @returns 寻找到的mes数据
 */
export function findNextChatWhitTableData(
	startIndex,
	isIncludeStartIndex = false
) {
	if (startIndex === -1) return { index: -1, chat: null };
	const chat = USER.getContext().chat;
	for (
		let i = isIncludeStartIndex ? startIndex : startIndex + 1;
		i < chat.length;
		i++
	) {
...
export function initTableData(eventData) {
	const allPrompt = USER.tableBaseSetting.message_template.replace(
		"{{tableData}}",
...
function isTableEditStrChanged(chat, matches) {
	if (
		chat.tableEditMatches != null &&
		chat.tableEditMatches.join("") === matches.join("")
	) {
		return false;
	}
 ...
 * @returns
 */
export function handleEditStrInMessage(
	chat,
	mesIndex = -1,
	ignoreCheck = false
) {
...
export function parseTableEditTag(piece, mesIndex = -1, ignoreCheck = false) {
	...
	tableEditActions.forEach(
		(action, index) =>
			(tableEditActions[index].action = classifyParams(
	...
	switch (EditAction.type) {
		case "update":
			...
			if (rowIndex >= sheet.getRowCount() - 1)
				return executeAction({ ...EditAction, type: "insert" }, sheets);
			if (!action?.data) return;
			Object.entries(action.data).forEach(([key, value]) => {
				const cell = sheet.findCellByPosition(
					rowIndex + 1,
					parseInt(key) + 1
				);
				if (!cell) return -1;
...
export function replaceTableEditTag(chat, newContent) {
	...
	if (/<tableEdit>.*?<\/tableEdit>/gs.test(chat.mes)) {
		chat.mes = chat.mes.replace(
			/<tableEdit>(.*?)<\/tableEdit>/gs,
	...
	if (chat.swipes != null && chat.swipe_id != null)
		if (/<tableEdit>.*?<\/tableEdit>/gs.test(chat.swipes[chat.swipe_id])) {
			chat.swipes[chat.swipe_id] = chat.swipes[chat.swipe_id].replace(
				/<tableEdit>(.*?)<\/tableEdit>/gs,
				`<tableEdit>\n${newContent}\n</tableEdit>`
			);
		} else {
			...
			if (
				...
				if (tableData) {
					...
					if (USER.tableBaseSetting.deep === 0) {
						...
						eventData.chat.splice(-USER.tableBaseSetting.deep, 0, {
							role: getMesRole(),
							content: finalPrompt,
						});
					}
...
function getMacroPrompt() {
	try {
		if (
			...
			return "";
		if (USER.tableBaseSetting.step_by_step === true) {
			const promptContent = replaceUserTag(
				getTablePrompt(undefined, true)
			);
...
async function onMessageEdited(this_edit_mes_id) {
	...
	if (chat.is_user === true || USER.tableBaseSetting.isAiWriteTable === false)
		return;
	try {
...
function resolveTableMacros(text) {
	...
	return text.replace(
		...
		(match, tableName, cellAddress) => {
			...
			const targetTable = sheets.find(
				(t) => t.name.trim() === tableName.trim()
			);
...
async function onMessageSwiped(chat_id) {
	if (
		USER.tableBaseSetting.isExtensionAble === false ||
		USER.tableBaseSetting.isAiWriteTable === false
	)
		return;
	const chat = USER.getContext().chat[chat_id];
...
jQuery(async () => {
	...
	$(".extraMesButtons").append(
		'<div title="查看表格" class="mes_button open_table_by_id">表格</div>'
	);
	...
	// 添加顶部表格管理工具弹窗
	$("#extensions-settings-button").after(
		await SYSTEM.getTemplate("appHeaderTableDrawer")
	);
	...
	// 监听主程序事件
	APP.eventSource.on(
		APP.event_types.CHARACTER_MESSAGE_RENDERED,
		onMessageReceived
	);
	APP.eventSource.on(
		APP.event_types.CHAT_COMPLETION_PROMPT_READY,
