...
export const USER = {
	...
	getChatPiece: (deep = 0, direction = "up") => {
		...
		if (!chat || chat.length === 0 || deep >= chat.length)
	...
	IMPORTANT_USER_PRIVACY_DATA: createProxyWithUserSetting(
...
export const BASE = {
		...
		{},
		{
			...
			set(_, target, value) {
				switch (target) {
					...
					default:
						throw new Error(
							`Cannot set sheetsData target: ${target}`
						);
				}
			},
		}
	...
	copyHashSheets(hashSheets) {
		...
		for (const sheetUid in hashSheets) {
			...
			newHashSheet[sheetUid] = hashSheet.map((row) =>
				row.map((hash) => hash)
			);
		}
		return newHashSheet;
	...
	applyJsonToChatSheets(json, type = "both") {
		const newSheets = Object.entries(json)
			.map(([sheetUid, sheetData]) => {
		...
		const oldSheets = BASE.getChatSheets().filter(
	...
	getLastSheetsPiece(
		...
		for (
			let i = startIndex;
			direction === "up"
				? i >= 0 && i >= startIndex - cutoff
				: i < chat.length && i < startIndex + cutoff;
			direction === "up" ? i-- : i++
		) {
	...
	getReferencePiece() {
		...
		const { piece } = swipeInfo.isSwipe
			? swipeInfo.deep === -1
				...
				: BASE.getLastSheetsPiece(swipeInfo.deep - 1, 1000, false)
	...
	hashSheetsToSheets(hashSheets) {
		...
		return BASE.getChatSheets((sheet) => {
			if (hashSheets[sheet.uid]) {
				sheet.hashSheet = hashSheets[sheet.uid].map((row) =>
					row.map((hash) => hash)
				);
...
export const EDITOR = {
	...
	info: (message, detail = "", timeout = 500) =>
		consoleMessageToEditor.info(message, detail, timeout),
	success: (message, detail = "", timeout = 500) =>
		consoleMessageToEditor.success(message, detail, timeout),
	warning: (message, detail = "", timeout = 2000) =>
		consoleMessageToEditor.warning(message, detail, timeout),
	error: (message, detail = "", error, timeout = 2000) =>
		consoleMessageToEditor.error(message, detail, error, timeout),
	clear: () => consoleMessageToEditor.clear(),
	logAll: () => {
		SYSTEM.codePathLog(
			{
				user_table_database_setting: USER.getSettings().muyoo_dataTable,
				user_tableBase_templates:
					USER.getSettings().table_database_templates,
				context: USER.getContext(),
				context_chatMetadata_sheets:
					USER.getContext().chatMetadata?.sheets,
...
export const SYSTEM = {
	getTemplate: (name) => {
		...
		return APP.renderExtensionTemplateAsync(
			"third-party/st-memory-enhancement/assets/templates",
			name
		);
	},
