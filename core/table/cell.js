...
export class Cell {
		...
		if (this.parent?.constructor?.name === "SheetTemplate") {
			if (rowIndex === 0 && colIndex === 0) {
				...
				this.element.textContent =
					this.data.value || getColumnLetter(colIndex - 1); // Column headers (A, B, C...)
				this.element.classList.add("sheet-header-cell-top");
			} else if (colIndex === 0) {
				if (
					this.parent.type === SheetBase.SheetType.dynamic ||
					this.parent.type === SheetBase.SheetType.fixed
				) {
				...
				if (this.parent.type === SheetBase.SheetType.static) {
					const pos = [getColumnLetter(colIndex - 1), rowIndex].join(
						""
					); // Cell position (A1, B2, C3...)
	...
	#init(target) {
		...
		this.uid =
			targetCell.uid ||
			`cell_${
				this.parent.uid.split("_")[1]
			}_${SYSTEM.generateRandomString(16)}`;
		this.coordUid =
			targetCell.coordUid || `coo_${SYSTEM.generateRandomString(15)}`;
	...
	#event(actionName, props = {}, isSave = true) {
		...
		switch (actionName) {
			...
			case CellAction.clearSheet:
				...
				break;
			default:
				debugWarn(`未处理的单元格操作: ${actionName}`);
		}

		// 触发自定义事件监听器
		if (this.customEventListeners[actionName]) {
			this.customEventListeners[actionName].forEach((callback) => {
				// 遍历执行数组中的回调函数
				callback(this, actionName, props); // 传递 cell 实例, actionName, 和 props
			});
		}
		if (this.customEventListeners[""]) {
			this.customEventListeners[""].forEach((callback) => {
				// 遍历执行数组中的回调函数
		...
		debugLog(`单元格操作: ${actionName} 位置: ${[rowIndex, colIndex]}`);
	}
	#handleEditCell(props = {}) {
		if (!props || Object.keys(props).length === 0) {
			debugWarn("未提供任何要修改的属性");
			return;
		}
		...
		debugLog("保存前的 cell", this.parent.cellHistory);
	...
	#insertRow(targetRowIndex) {
		...
		const newRow = Array.from(
			{ length: this.parent.hashSheet[0].length },
			(_, j) => {
	...
	#deleteRow(rowIndex) {
		debugLog("删除行", rowIndex, this.parent.hashSheet.length);
		if (rowIndex === 0) return;
