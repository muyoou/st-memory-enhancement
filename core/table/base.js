...
export class SheetBase {
		...
		this.positionCache = new Proxy(new Map(), {
			get: (map, uid) => {
				if (this._cellPositionCacheDirty || !map.has(uid)) {
					...
					debugLog("重新计算 positionCache: ", map);
				}
				return map.get(uid);
			},
		});
	...
	markPositionCacheDirty() {
		...
		// debugLog(`标记 Sheet: ${this.name} (${this.uid}) 的 positionCache 为脏`);
	}
	...
	init(column = 2, row = 2) {
		...
		// 初始化 hashSheet 结构
		const r = Array.from({ length: row }, (_, i) =>
	...
	rebuildHashSheetByValueSheet(valueSheet) {
		...
		const newHashSheet = Array.from({ length: rows }, (_, i) =>
	...
	findCellByValue(value, cellType = null) {
		const cell = this.cellHistory.find(
			(cell) =>
	...
	findCellByPosition(rowIndex, colIndex) {
		if (
			rowIndex < 0 ||
			colIndex < 0 ||
			rowIndex >= this.hashSheet.length ||
			colIndex >= this.hashSheet[0].length
		) {
			debugWarn("无效的行列索引");
			return null;
		}
		...
		if (!target) {
			...
			cell.type =
				colIndex === 0
					? Cell.CellType.row_header
					: rowIndex === 0
					? Cell.CellType.column_header
					: Cell.CellType.cell;
			cell.uid = hash;
			this.cells.set(cell.uid, cell);
			return cell;
		}
		debugLog("找到单元格", target);
		return target;
	}
	/**
	 * 通过行号获取行的所有单元格
	...
	getSheetCSV(removeHeader = true, key = "value") {
		...
		debugLog("测试获取map", this.cells);
		const content = this.hashSheet
			.slice(removeHeader ? 1 : 0)
			.map((row, index) =>
				row
					.map((cellUid) => {
						const cell = this.cells.get(cellUid);
						if (!cell) return "";
						return cell.type === Cell.CellType.row_header
							? index
							: cell.data[key];
					})
					.join(",")
