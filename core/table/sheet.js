...
export class Sheet extends SheetBase {
	 ...
	 * */
	renderSheet(
		cellEventHandler = this.lastCellEventHandler,
		targetHashSheet = this.hashSheet,
		lastCellsHashSheet = null
	) {
		this.lastCellEventHandler = cellEventHandler;

		// 预先计算渲染所需的数据副本，避免修改实际的 this.hashSheet
		const currentHashSheet = Array.isArray(targetHashSheet)
			? targetHashSheet
			: this.hashSheet || [];
		...
		if (
			!prevHashSheetsMap &&
			typeof DERIVED?.any?.renderDeep === "number" &&
			DERIVED.any.renderDeep !== 0
		) {
			try {
				prevHashSheetsMap = BASE.getLastSheetsPiece(
					DERIVED.any.renderDeep - 1,
					3,
					false
				)?.piece?.hash_sheets;
				if (prevHashSheetsMap)
					prevHashSheetsMap = BASE.copyHashSheets(prevHashSheetsMap);
			} catch (_) {
				// 忽略获取失败，保持无高亮
			}
		}
		...
		const changeSheet = renderHashSheet.map((row) => {
			...
			return row.map((hash) => {
				...
				if (!lastHashSheetFlat.includes(hash))
					return { hash, type: "update" };
		...
		let isKeepAllCol = Array.from(
			{ length: changeSheet[0].length },
			(_, i) => (i < 2 ? false : true)
		);
	...
	save(targetPiece = USER.getChatPiece()?.piece, manualSave = false) {
		...
		try {
			if (sheets.some((t) => t.uid === sheetDataToSave.uid)) {
				sheets = sheets.map((t) =>
					t.uid === sheetDataToSave.uid ? sheetDataToSave : t
				);
			} else {
	...
	getTableText(
		...
		const node =
			this.source.data.note && this.source.data.note !== ""
				? "【说明】" + this.source.data.note + "\n"
				: "";
		const headers =
			"rowIndex," +
			this.getCellsByRowIndex(0)
				.slice(1)
	...
	#load(target) {
		...
		if (typeof target === "string") {
			let targetSheetData = BASE.sheetsData.context?.find(
				(t) => t.uid === target
			);
	...
	#getTableEditRules() {
		...
		if (this.required && this.isEmpty())
			...
			if (source.data.insertNode)
				editRules += "插入：" + source.data.insertNode + "\n";
			if (source.data.updateNode)
				editRules += "更新：" + source.data.updateNode + "\n";
			if (source.data.deleteNode)
				editRules += "删除：" + source.data.deleteNode + "\n";
			return editRules;
		}
	}
...
function getLatestChatHistory(chat, deep) {
	...
	for (let i = 0; i < Math.min(deep, floors); i++) {
		// 格式化消息并清理标签
		const currentStr = `${filteredChat[floors - i - 1].mes}`.replace(
