...
export function parseManualJson(jsonStr) {
	...
	try {
		...
		debugLog("手动解析 JSON 成功:", result);
		return result;
	} catch (error) {
		debugError("JSON 解析错误:", error.message);
		throw error;
	}
}
...
export function safeParse(jsonStr) {
	...
	while (startIndex < jsonStr.length) {
		...
		if (bracketEnd !== -1) {
			// 提取JSON数组字符串
			const jsonArrayStr = jsonStr.substring(
				bracketStart,
				bracketEnd + 1
			);
			debugLog("发现JSON数组:", jsonArrayStr);

			try {
				// 优先使用原生 JSON.parse
				const parsed = JSON.parse(jsonArrayStr);
				results.push(parsed);
				debugLog("成功解析JSON数组:", parsed);
			} catch (error) {
				debugWarn("原生 JSON.parse 失败，尝试手动解析:", error.message);
				try {
					const parsed = parseManualJson(jsonArrayStr);
					results.push(parsed);
					debugLog("手动解析成功:", parsed);
				} catch (manualError) {
					debugError("手动解析也失败:", manualError.message);
					// 继续查找下一个数组，不抛出错误
				}
			}
	...
	if (results.length === 0) {
		debugWarn("未找到有效的JSON数组");
		return [];
	}

	debugLog(`总共解析出 ${results.length} 个JSON数组:`, results);
	return results;
}
